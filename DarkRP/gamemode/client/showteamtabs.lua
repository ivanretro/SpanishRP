...
local function MayorOptns()
	...
	MayCat:SetLabel(LANGUAGE.menu_mayor)
...
local function CPOptns()
	...
	CPCat:SetLabel(LANGUAGE.menu_police)
...
local function CitOptns()
	...
	CitCat:SetLabel(LANGUAGE.menu_citizen)
...
local function MobOptns()
	...
	MobCat:SetLabel(LANGUAGE.menu_mobboss)
...
function GM:MoneyTab()
	...
	FirstTabPanel:EnableVerticalScrollbar( true )
		function FirstTabPanel:Update()
			...
			MoneyCat:SetLabel(LANGUAGE.money)
			...
			Commands:SetLabel(LANGUAGE.actions)
				...
				local UnOwnAllDoors = ActionsPanel:Add("DButton")
						UnOwnAllDoors:SetText(LANGUAGE.sell_doors)
		...
		local function AddIcon(Model, name, description, Weapons, command, special, specialcommand)
			...
			icon.DoClick = function()
				local function DoChatCommand(frame)
					if special then
						local menu = DermaMenu()
						menu:AddOption(LANGUAGE.vote, function() LocalPlayer():ConCommand("darkrp "..command) frame:Close() end)
						menu:AddOption(LANGUAGE.not_vote, function() LocalPlayer():ConCommand("darkrp " .. specialcommand) frame:Close() end)
						menu:Open()
					else
				...
				if type(Model) == "table" and #Model > 0 then
					...
					frame:SetTitle(LANGUAGE.menu_model)
...
function GM:EntitiesTab()
	...
	EntitiesPanel:EnableVerticalScrollbar( true )
		function EntitiesPanel:Update()
			...
			AmmCat:SetLabel(LANGUAGE.menu_ammo)
			...
			WepCat:SetLabel(LANGUAGE.menu_weapons)
			...
			ShipCat:SetLabel(LANGUAGE.menu_shipments)
			...
			EntCat:SetLabel(LANGUAGE.menu_ent)
				...
				EntPanel:EnableVerticalScrollbar(true)
					...
					for k,v in pairs(DarkRPEntities) do
						if not v.allowed or (type(v.allowed) == "table" and table.HasValue(v.allowed, LocalPlayer():Team()))
							...
							AddEntIcon(v.model, LANGUAGE.buy_a .. v.name .." " .. GAMEMODE.Config.currency .. v.price, v.cmd)
							entnum = entnum + 1
						end
					end
			...
			VehicleCat:SetLabel(LANGUAGE.menu_vehicles)
...
function GM:RPHUDTab()
	...
	function HUDTABpanel:Update()
		...
		backgrndcat:SetLabel("HUD background")
			...
			resetbackgrnd:SetText(LANGUAGE.reset)
		...
		hforegrndcat:SetLabel("Health bar foreground")
			...
			resethforegrnd:SetText(LANGUAGE.reset)
		...
		hbackgrndcat:SetLabel("Health bar Background")
			...
			resethbackgrnd:SetText(LANGUAGE.reset)
		...
		hTextcat:SetLabel("Health bar text")
			...
			resethText:SetText(LANGUAGE.reset)
		...
		jobs1cat:SetLabel("Jobs/wallet foreground")
			...
			resetjobs1:SetText(LANGUAGE.reset)
		...
		jobs2cat:SetLabel("Jobs/wallet background")
			...
			resetjobs2:SetText(LANGUAGE.reset)
		...
		salary1cat:SetLabel("Salary foreground")
			...
			resetsalary1:SetText(LANGUAGE.reset)
		...
		salary2cat:SetLabel("Salary background")
			...
			resetsalary2:SetText(LANGUAGE.reset)
		...
		local HudWidthpanel = vgui.Create("DListLayout")
			...
			resetHudWidth:SetText(LANGUAGE.reset)
		...
		local HudHeightpanel = vgui.Create("DListLayout")
			...
			resetHudHeight:SetText(LANGUAGE.reset)
